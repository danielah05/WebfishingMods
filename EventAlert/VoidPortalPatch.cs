...
public class VoidPortalPatch : IScriptMod {
    ...
    public bool ShouldRun(string path) => path == "res://Scenes/Entities/VoidPortal/void_portal.gdc";
        ...
        foreach (var token in tokens) {
            if (readyMatch.Check(token)) {
                ...
                if (Mod.Config.ShowLogs) {
                    ...
                    yield return new Token(TokenType.ParenthesisClose);
                    yield return new Token(TokenType.Newline, 1);
                    // Network._update_chat("[color=#1e814e](" + time + " EventAlert)[/color]: a void portal has opened!")
                    ...
                    yield return new IdentifierToken(Time);
                    yield return new Token(TokenType.OpAdd);
                    yield return new ConstantToken(new StringVariant(" EventAlert)[/color]: a void portal has opened!"));
                    yield return new Token(TokenType.ParenthesisClose);
                    yield return new Token(TokenType.Newline, 1);
                }
